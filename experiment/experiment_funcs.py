...
def train_val_wandb(model, train_loader, val_loader, criterion, optimizer, num_classes=3, num_epochs=10, device="mps", fold=0):
    best_val_acc = 0.0
    fold_val_losses = []
    ...
    for epoch in range(num_epochs):
        ...
        train_metrics = compute_metrics(all_preds, all_labels, num_classes=num_classes)
        ...
        val_metrics = compute_metrics(val_preds, val_labels, num_classes=num_classes)
...
def sweep_train(model_class, device, k, dataset, freeze_type):
    
    run = wandb.init(
        project="CNN_KFold_HyperSweep",
    )
    config = wandb.config
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(np.arange(len(dataset)))):
        ...
        avg_val_loss = train_val_wandb(model=model, 
