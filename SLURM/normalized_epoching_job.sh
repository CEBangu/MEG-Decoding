...
if [ "$SAVE_BASELINE" == "1" ]; then
    python3 $HOME/MEG-Decoding/normalized_epoching.py --subject "$SUBJECT" --root "$ROOT" --save_baseline
else
    python3 $HOME/MEG-Decoding/normalized_epoching.py --subject "$SUBJECT" --root "$ROOT"
fi