...
files = sorted(glob.glob("percentiles_OP/percentile_*.csv"))
...
files = sorted(glob.glob("percentiles_CP/percentile_*.csv"))
df = pd.concat([pd.read_csv(f, header=None) for f in files])
...
files = sorted(glob.glob("percentiles_CR/percentile_*.csv"))
df = pd.concat([pd.read_csv(f, header=None) for f in files])
...
print("Covert Producing")
for roi in rois:
    files = sorted(glob.glob(f"percentiles_roi_CP/roi_{roi}_*.csv"))
    if not files:
        continue
    df = pd.concat([pd.read_csv(f, header=None) for f in files])
...
for roi in rois:
    files = sorted(glob.glob(f"percentiles_roi_CR/roi_{roi}_*.csv"))
    if not files:
        continue
    df = pd.concat([pd.read_csv(f, header=None) for f in files])
    print(f"{roi.upper()}: min= {df[0].mean():.4f}, max= {df[1].mean():.4f}")
...
print("Overt Producing")
for roi in rois:
    files = sorted(glob.glob(f"percentiles_roi_OP/roi_{roi}_*.csv"))
    if not files:
        continue
    df = pd.concat([pd.read_csv(f, header=None) for f in files])
    print(f"{roi.upper()}: min= {df[0].mean():.4f}, max= {df[1].mean():.4f}")